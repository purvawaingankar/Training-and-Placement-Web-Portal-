...
if(!preg_match('/^[A-Za-z0-9._%+-]+@nhitm\.ac\.in$/', $email)){
  // Email is valid and from the allowed domain
  // Do whatever you want with the email address
  $error[] =  "Only email addresses from the nhitm.ac.in domain are allowed.";
}
   if($passwordConfirm ==''){
...
$row = mysqli_fetch_assoc($res);
    ...
    $password = password_hash($password,PASSWORD_BCRYPT,$options);
            ...
            $result = mysqli_query($dbc,"INSERT into admins(fname,lname,username,email,password,date) values('$fname','$lname','$username','$email','$password','$date')");
...
?>
      ...
      <?php if(isset($done)) 
      { ?>
    <div class="successmsg"><span style="font-size:100px;">&#9989;</span> <br> You have registered successfully . <br> 
    <a href="login_admin.php" style="color:black;">Login here... </a> </div>
      <?php } else { ?>
        <h3> Admin's Sigup </h3><br>
...
<div class="form-group">
  ...
  <button type="submit" name="signup" class="btn btn-primary btn-group-lg form_btn">SignUp</button>
   <p>Have an account?  <a href="login_admin.php">Log in</a> </p>
</form>
<?php } ?> 
</div>
		</div>
